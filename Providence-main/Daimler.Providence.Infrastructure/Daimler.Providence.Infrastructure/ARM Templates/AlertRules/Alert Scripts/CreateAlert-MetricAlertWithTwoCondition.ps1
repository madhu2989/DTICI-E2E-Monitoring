...
param (
    ...
    #Alert Rule Condition Name
    [Parameter(Mandatory=$true)]
    [String] $MeticRuleName1,

    #Metric Rule Time Aggregation
    [Parameter(Mandatory=$true)]
    [String] $TimeAggregation1,
...
try{
    ...
    #Write-Host 'Install Az modules'
    #Import-Module Az.Resources
    Write-Host "Uninstalling Azure RM module"
    Uninstall-AzureRm
    #Write-Host 'Getting Available Module List'
   ...
   Write-Host "Getting the target scope"
    ...
    $AlertTemplateFile = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($PSScriptRoot, "..\MetricAlertTwoCondition\", $AlertTemplateFile))
    $AlertTemplateParametersFile = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($PSScriptRoot, "..\MetricAlertTwoCondition\", $AlertTemplateParametersFile))
    ...
    $AlertOverwriteTemplateParameters.Add("Threshold1", $Threshold1)
    $AlertOverwriteTemplateParameters.Add("MetricName1", $MeticRuleName1)
    $AlertOverwriteTemplateParameters.Add("Operator1", $Operator1)
    $AlertOverwriteTemplateParameters.Add("TimeAggregation1", $TimeAggregation1)
